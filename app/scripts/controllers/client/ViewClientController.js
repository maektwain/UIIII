(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewClientController: function (scope, routeParams, route, location, resourceFactory, http, $modal, API_VERSION, $rootScope, $upload, $window) {
            scope.client = [];
            ...
            scope.getClientAddress = function(){
                resourceFactory.clientResource.getAllAddresses({clientId : routeParams.id, anotherresource:'addresses'}, function(data){
                    ...
                    if (scope.datatabledetails.isData) {
                        for (var i in data.columnHeaders) {
                            if (!scope.datatabledetails.isMultirow) {
                                ...
                                if (clientdatatable.registeredTableName == "Official"){
                                    console.log(clientdatatable.registeredTableName)

                                    if (data.data[0].row[7].length == 6){
                                        scope.singleRow.push(row);
                                }else {

                                    return scope.showError = "Error in Pin Code";
                                }

                                }
                                if (clientdatatable.registeredTableName == "Banking Details"){

                                    scope.singleRow.push(row);
                                }
                                if(clientdatatable.registeredTableName == "Banking"){
                                    scope.singleRow.push(row);
                                }
                                if(clientdatatable.registeredTableName == "Permanent Address"){
                                    scope.singleRow.push(row);
                                }
                                if(clientdatatable.registeredTableName == "Present Address"){
                                    scope.singleRow.push(row);
                                }
                            }
                        }
    ...
    mifosX.ng.application.controller('ViewClientController', ['$scope', '$routeParams', '$route', '$location', 'ResourceFactory', '$http', '$modal', 'API_VERSION', '$rootScope', '$upload', '$window',mifosX.controllers.ViewClientController]).run(function ($log) {
