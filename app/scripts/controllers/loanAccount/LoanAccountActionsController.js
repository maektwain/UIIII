    ...
    mifosX.controllers = _.extend(module, {
        LoanAccountActionsController: function (scope, resourceFactory, location, routeParams, dateFilter) {
            ...
            switch (scope.action) {
                case "approve":
                    resourceFactory.loanTemplateResource.get({loanId: scope.accountId, templateType: 'approval'}, function (data) {
                        ...
                        scope.formData.approvedLoanAmount =  scope.total;
                ...
                case "disburse":
                    ...
                    resourceFactory.loanTrxnsTemplateResource.get({loanId: scope.accountId, command: 'disburse'}, function (data) {
                        ...
                        scope.paymentToWhoms = data.paymentToWhomOptions;
                        if(data.paymentToWhomOptions.length > 0){
                            scope.formData.paymentToWhomId = data.paymentToWhomOptions[0].id;
                        }
            ...
            scope.checkQuestions=function() {
              if (scope.formData.approvedLoanAmount > scope.total ) {
                return true;
              }else {
                  return false;
                }
