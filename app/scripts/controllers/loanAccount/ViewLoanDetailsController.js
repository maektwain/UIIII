(function (module) {
    mifosX.controllers = _.extend(module, {
        ViewLoanDetailsController: function (scope, routeParams, resourceFactory, location, route, http, $modal, dateFilter, API_VERSION, $sce, $rootScope) {
            ...
            scope.$watch('loandetails', function () {
              $rootScope.total = 0
              angular.forEach(scope.loandetails.collateral, function(collateral){
                  $rootScope.total = $rootScope.total + collateral.value
              })
          }, true);
            ...
            resourceFactory.LoanAccountResource.getLoanAccountDetails({loanId: routeParams.id, associations: 'all',exclude: 'guarantors'}, function (data) {
                ...
                $rootScope.NetDisbursal = data.repaymentSchedule.totalPrincipalDisbursed - data.repaymentSchedule.totalFeeChargesCharged;
